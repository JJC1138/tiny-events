public class TinyEventWithData<T> {
    ...
    fileprivate func removeDeadObserver() {
        ...
        let observerIndex = observers.firstIndex(where: { $0.observer == nil })!
...
public class TinyEvent {
    ...
    public func add(_ callback: @escaping () -> Void) -> TinyEventObserver {
        let observer = TinyEventObserver(event: self, callback: callback)
        observers.append(WeakTinyEventObserver(observer: observer))
        return observer
    }
    ...
    // This struct only exists because we want to store weak references in the above array:
    private struct WeakTinyEventObserver {
        weak var observer: TinyEventObserver?
    }
    
}

public class TinyEventObserver {
    
    fileprivate init(event: TinyEvent, callback: @escaping () -> Void) {
        self.event = event
        self.callback = callback
    }
    
    fileprivate let callback: () -> Void
    private weak var event: TinyEvent?
